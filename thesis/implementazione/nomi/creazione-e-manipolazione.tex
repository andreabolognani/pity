\subsection{Creazione e manipolazione}
...
\lstinline{name}, che viene usato per rappresentare un nome. La
...
bene spiegare, visto che nell'implementare Pity ne viene fatto ampio uso.
...
Questa caratteristica viene utilizzata ampiamente nell'ambito dei
processi; per quanto riguarda i nomi, viene utilizzata solamente
per ragioni di consistenza.
...
hanno dettato questa scelta vengono spiegate poco pi\`u avanti.
...
Da questo momento in poi, il contratto di ogni procedura pubblica viene
dichiarato assieme alla procedura stessa, dando per scontato che la
sintassi \lstinline{provide/contract} sia usata per esportare la
